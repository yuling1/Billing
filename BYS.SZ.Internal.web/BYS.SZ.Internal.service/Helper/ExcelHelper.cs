...
namespace BYS.SZ.Internal.web.Service
{
    public class ExcelHelper : IExcelService, IDisposable
    {
        ...
        public List<List<object>> TableToEntities(string space, DataTable dt)
        {
            ...
            List<Type> types = typeof(db.DBContext.Basic_DbContext).Assembly.GetTypes().Where(x => x.Namespace.Contains(space)).ToList();//获取名为space的NameSpace下的所有Type
        ...
        public List<object> TableToEntity(DataTable dt, Type entityType)
        {
            ...
            {//依次遍历DataTable的每一行
                ...
                {//遍历一条数据的每个单元格
                    ...
                    //bool result = properties.Any(x =>
                    //{
                    //    var customAttribute = x.GetCustomAttribute<ColumnNameAttribute>();

                    //    if (customAttribute != null && customAttribute.Name == headName)
                    //    {//如果列名和ColumnNameAttribute相同，则返回true
                    //        matchingColumnName = x.Name;
                    //        return true;
                    //    }
                    //    else if (customAttribute == null && headName == x.Name)
                    //    {//如果列名和属性名相同，则返回true
                    //        return true;
                    //    }
                    //    else
                    ...
                    {//如果单元格的列名是Comments的话，将单元格的数据存到对象MetaData属性中
